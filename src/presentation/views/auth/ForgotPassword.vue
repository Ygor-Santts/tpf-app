...
async function submit(){
  ...
  try { await forgotPassword(email.value); message.value = t('auth.emailSent') } catch (e) { message.value = t('auth.emailSent') } finally { loading.value = false }
}
</script>
...
<template>
  <div class="max-w-xs mx-auto grid gap-4">
    ...
    <form @submit.prevent="submit" class="grid gap-3">
      ...
      <button :disabled="loading" class="rounded-xl p-3 bg-black text-white">
